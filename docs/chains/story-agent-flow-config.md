...
- 故事篇幅约定：每一个 Scene 要能拆出 8 到 16 张图。可入画动作不超过 16 个时只写 1 个 Scene；超过 16 个再拆场，每场仍是 8 到 16 个，例如 8 和 9，不要拆出薄场。导演必须输出 `SCENE_COUNT` 和 `BEAT_COUNTS`。作家每场写 5 到 8 个短段落。已保存的旧 Prompt 仍会在执行时追加同一协议。本版不改图片工作台。
- 年级可选：未指定或初中及以后不考虑超纲。小学目标用词不超过高中即可，不删 `cardboard`/`paw`/`sigh`；只有大学学术词或专业术语才算离谱超纲。执行器在未指定学段时忽略 grade 分。质量决策只接受首行 `ACTION: PASS|REVISE|REWRITE`；`REPLAN`/`REPITCH`/`REDIRECT` 视为非法。语言低于 3 或年级为 1 时先修订，只有修订过一轮仍阻断或同时缺词才重写。趣味低分只修订。`replaceWith` 必须保住可见动作，不得只加空情绪词，也不得削弱画面动词；画面动词可以互换。不合规条目忽略，全部不合规则交付当前最好一稿。修订后趣味或连续性下降则丢弃该稿。词已覆盖且语言不低于 3 时：`PASS` 且连续性也不低于 3 立即完成；已经改过且连续性不低于 3 也直接 `COMPLETED`；连续性仍低于 3 再允许一轮修订。交付本轮与历史里评分更高的一稿（同分取新稿）。作家第一段必须是可见冲突，猫用 `The cat`/`It`。`hard-rule-check`、`candidate-snapshot`、`budget-controller` 和 `human-review` 是画布拓扑节点：词扫描和最高分选择由执行器内联完成，这些节点不单独落步骤。最后一轮若仍有回退额度且尚未过线停，先执行再停。
- `story-writer` 和 `targeted-reviser` 的数据库 Prompt 仍可编辑，执行时另行追加不可编辑的 `STORY_TEXT_BEGIN` / `STORY_TEXT_END` 协议。原始响应完整保存在 `tb_story_run_step.output_text`；只有唯一边界内包含纯英文 `Scene N:` 标题和故事段落且不含 Markdown、中文说明、清单、表格或修订记录的故事块才进入审核链和 `tb_story_run.final_story`。结束边界外的报告只留在步骤原始输出；边界缺失/歧义或故事块内部格式错误会使步骤与批次失败，不回退为保存完整响应。
