...
- 一级导航包含“Agent 工作台”。工作台画布固定展示策划与创意、写作与候选、独立质量委员会、修订与交付四个阶段，全部节点均可点击查看。`hard-rule-check`、`candidate-snapshot`、`budget-controller` 和 `human-review` 是只读拓扑节点，不单独执行模型调用。
...
- 工作台页头提供“开始运行”和“运行记录”。开始运行可选目标学段：不限制、小学、初中、高中、大学；不选则不考虑超纲。支持逐行输入单词，或从已保存的外部词库随机预览后创建正式批次。
...
- 顶部“英语素材项目”是可展开入口，子菜单依次包含“Agent 生成结果”和“图片生成结果”。Agent 结果页只展示 `COMPLETED` 且最终故事非空的故事批次，按创建时间倒序执行服务端分页；默认每页 10 条，可切换 20 或 100 条。每条结果占满内容区宽度，上层单行显示首个 Scene 标题、年级、单词数和生成时间，下层按自然高度完整展示故事并可复制全文；不展示 Agent 中间输出、评分、Token、筛选或删除操作。
- 图片结果页只展示 `COMPLETED` 且含 `FINAL` 资产的图片批次，同样使用 10、20、100 条服务端分页。每个批次占满内容区宽度，标题单行汇总故事标题、画风、最终图片数、学段和完成时间；下层按 Scene/Shot 顺序自适应排列 16:9 最终图片卡片，展示对白、字幕或来源摘要并支持大图预览。该页不展示参考图、底图、Agent/程序步骤、Token、评分或失败批次，所有图片只通过受控资产 ID 接口加载。
...
- 一级导航在“Agent 工作台”后提供独立“图片工作台”。配置页固定展示 4 个阶段、9 个可编辑文本 Agent 和 3 个只读程序节点；节点上下游由后端 Catalog 随流程 API 返回，前端不再维护第二份边表。Agent 可编辑 Prompt、文本 Provider、Temperature 和启用状态，并可查看或恢复追加式 Prompt 版本。
...
- “开始生成”只允许从后端返回的已有最终故事和启用画风中选择，预览学段（空则显示不限制）、单词和完整故事；九个 Agent、画风、图片 Provider 或故事任一不合格时禁止提交，提交期间防止重复创建。
...
主要请求包括 `/connection/*`、`/ai/config`、`POST /ai/config/image/bootstrap`、`/ai/cli/config`、`/minio-config*`、`/story-agents/*`、`/story-runs*`（其中 `GET /story-runs/results` 只接受 10/20/100 的分页大小）、`/image-agents/*`、`/image-style-presets*`、`/image-runs*`（其中 `GET /image-runs/results` 同样只接受 10/20/100）、`/image-assets/{id}/content` 和 `/word-clean*`。图片字节仍通过后端资产 ID 接口读取，浏览器不直接访问 MinIO。
