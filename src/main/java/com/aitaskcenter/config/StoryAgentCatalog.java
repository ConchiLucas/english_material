...
public final class StoryAgentCatalog {
    ...
    private static final String STORY_WRITER_PROMPT = """
            ...
            必须自然使用全部目标词，允许策划认可的时态、单复数等词形变化，不得为凑词写无关句。输出完整故事正文、
            场景结构以及逐个目标词的实际词形和出现位置清单。若收到重写反馈，应重写正文但保留仍然有效的蓝图约束，
            不得自行修改评分、通过线或路由决定。
            """.strip();
    ...
    private static final String TARGETED_REVISER_PROMPT = """
            ...
            输出修订后的完整英文故事正文、更新后的目标词使用位置清单，以及逐项对应问题编号的变更记录。若某项要求互相冲突，
            明确标记而不要擅自扩大修改范围。不得修改分数、量表、通过线、预算或流程路由。
            """.strip();
