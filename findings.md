...
- 独立审查实证指出首版严格解析仍接受重复/粘连边界、带 `Scene 1` 的 Target Words/Score/Changes、下划线 Markdown、无首尾竖线表格和西里尔字母，必须逐项单独测试和拒绝。
- 格式失败的 Token 必须按实际调用恰好结算一次；定向修订失败时应保留上一个已校验故事，但批次状态为 `FAILED`。
- 语言审核输入仍写着“由故事正文中的使用位置清单提供”，与纯故事新契约冲突，应改为直接从 `candidateStory` 核对目标词。
- 第二次复审还发现 Markdown 链接、引用、编号列表、行内代码、两列表格和 `Revision Notes:` 可漏过；同时第一轮符号过滤会误伤 `$5`，需分别增加拒绝与接受用例。
- 第二轮修复后，故事块会拒绝任意竖线、Markdown 链接/引用/列表/代码和扩展审校标题；字符检查只拒绝非 Latin 字母与 OTHER_SYMBOL，正常美元符号和英文标点可保留。
- 审计标题不能只拒绝带冒号形式；模型常输出独立一行的 `Target Words`、`Rating`、`Review Notes`，因此解析器同时拒绝已识别标题的“行末”与“冒号”两种形式，且水平空白匹配不能跨行。
- 真实批次 `dcb6e1fa-f3d5-48ab-8c1c-3c95679fbbf3` 的修订 Agent 在唯一边界内输出了干净故事，却在结束边界后追加中文清单与变更记录；这些内容适合保留在步骤原始输出，不应进入最终结果，也不应让已有干净故事块失效。
- 修复后的真实批次验证了兼容策略：第二次 `targeted-reviser` 原始输出确有结束边界外报告，但步骤状态为 `COMPLETED`，下游只收到故事块，最终结果未包含任何报告内容。
