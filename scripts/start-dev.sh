...
nohup env TASK_CENTER_SERVER_PORT="$BACKEND_PORT" TASK_CENTER_DB_URL="jdbc:postgresql://${DB_HOST}:${DB_PORT}/${DB_NAME}" TASK_CENTER_DB_USER="$DB_USER" TASK_CENTER_DB_PASSWORD="$DB_PASSWORD" java -jar "$ROOT_DIR/target/ai-task-center-0.0.1-SNAPSHOT.jar" >"$RUNTIME_DIR/logs/backend.log" 2>&1 &
